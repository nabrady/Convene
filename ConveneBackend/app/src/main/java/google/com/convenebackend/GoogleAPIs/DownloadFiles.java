...
public class DownloadFiles extends AsyncTask<Double, Void, String> {
    ...
    private mapsFragment myMapActivity;


    public DownloadFiles( mapsFragment mapActivity ) {
...
//execute search
    ...
    protected void onPostExecute(String result) {
        placeMarkers=myMapActivity.getPlaceMarkers();
        ...
        if (places != null) {
            for (int p = 0; p < places.length; p++) {
                ...
                if (places[p] != null) {
                    ...
                    placeMarkers[p]= myMapActivity.getMap().addMarker(places[p]);
                    Log.d("MARKERADD", "COMP");
                    x=p;
                }
            }
            if(places[0]!=null){
                myMapActivity.getMap().moveCamera(CameraUpdateFactory.newLatLng(places[0].getPosition()));
                myMapActivity.getMap().moveCamera(CameraUpdateFactory.zoomOut());
            }
        }
    }
}
